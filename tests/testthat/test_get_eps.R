...
col_names <- c("country_name", "country_id", "country_text_id",
	...
	"v2eltype_6","v2clstown_osp_codelow","v2clstown_osp_codehigh")

new_cols <- c("country_id", "country_text_id", "country_name",
	"year", "v2x_regime", "v2clstown_osp", "v2clstown_osp_codelow",
	"v2clstown_osp_codehigh", "reg_start_year", "reg_end_year",
...
test_that("Check column type output", {
	...
	expect_equal(class(get_eps()$v2clstown_osp_codelow), class(vdemdata::vdem$v2clstown_osp_codelow))
	expect_equal(class(get_eps()$v2clstown_osp_codehigh), class(vdemdata::vdem$v2clstown_osp_codehigh))
	})
...
test_that("Missingness in new variables", {
	...
	expect_equal(sum(is.na(get_eps(data = df)$v2clstown_osp)), sum(is.na(df$v2clstown_osp)))
	})
