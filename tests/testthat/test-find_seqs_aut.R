...
test_that("Invalid function arguments", {
    expect_error(find_seqs_sta(c(1, 2), 1))
    expect_error(find_seqs_sta(1, 1, start_incl = 1))
    expect_error(find_seqs_sta(1, 1, year_turn = -1))
    expect_error(find_seqs_sta(1, 1, cum_turn = -1))
    expect_error(find_seqs_sta(1, 1, tolerance = 0))
})
...
test_that("Basic decreasing seq", {
    ...
    out <- find_seqs_sta(polyarchy, regime)
    expect_equal(out, rep(1, 10))

    # Increase the start_incl threshold
    out <- find_seqs_sta(polyarchy, regime, start_incl = -2)
...
test_that("Multiple sequences", {
    ...
    out <- find_seqs_sta(polyarchy, regime, cum_turn = 2, year_turn = 2)
    expect_equal(out, c(1, 1, 1, 1))

    polyarchy <- c(2:1, 2, 3, 3)
    regime <- rep(1, 5)

    out <- find_seqs_sta(polyarchy, regime, year_turn = 1, cum_turn = 1)
...
test_that("Stasis", {
    ...
    out <- find_seqs_sta(polyarchy, regime, tolerance = 1)
    expect_equal(out, c(1, 1, NA, NA, NA))

    out <- find_seqs_sta(polyarchy, regime, tolerance = 2)
    expect_equal(out, c(1, 1, 1, NA, NA))

    # TODO: same issue as find_seqs_pri
    out <- find_seqs_sta(polyarchy, regime, tolerance = 3)
    ...
    out <- find_seqs_sta(polyarchy, regime, year_turn = 2, cum_turn = 2, tolerance = 2)
